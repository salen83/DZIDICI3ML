...
function saveSnapshot(screen1Teams = [], screen1Leagues = [], sofaRows = []) {
  ...
  } catch (e) { console.warn("⚠️ Greška pri čitanju snapshot-a:", e.message); }
  ...
  sofaRows.forEach(r => {
    Object.keys(r._data).forEach(k=>{
      ...
      if (!val) return;
      if (key.includes("home") || key.includes("away") || key.includes("team") || key.includes("domacin") || key.includes("gost")) teams.add(val);
      if (key.includes("league") || key.includes("tournament") || key.includes("competition") || key.includes("liga")) leagues.add(val);
    });
  });
  snapshot.sofa.teams = Array.from(teams);
  snapshot.sofa.leagues = Array.from(leagues);

  snapshot.screen1.teams = Array.from(new Set([...snapshot.screen1.teams, ...screen1Teams]));
  snapshot.screen1.leagues = Array.from(new Set([...snapshot.screen1.leagues, ...screen1Leagues]));
...
export default function SofaScreen({ onClose }) {
  ...
  const handleCellChange = (rowIdx, col, value) => {
    const copy = [...sofaRows];
    copy[rowIdx] = { ...copy[rowIdx], _data: { ...copy[rowIdx]._data, [col]: value } };
    setSofaRows(copy);
    saveSnapshot([], [], copy);
  };
