...
import FailedMappingPanel from "../components/FailedMappingPanel";
...
const handleDeleteSofaMatch = async (sofaId) => {

    const ok = window.confirm(
      `Obrisi Sofa mec ID ${sofaId} ?`
    );
    ...
    setFailedMappings(prev =>
      prev.filter(m => m.id !== sofaId)
    );

    setCurrentFailIndex(0);

    console.log("✅ Sofa match obrisan:", sofaId);
  };
...
<FailedMappingPanel
  ...
  setFailedMappings={setFailedMappings}
  currentFailIndex={currentFailIndex}
  setCurrentFailIndex={setCurrentFailIndex}
  onDelete={handleDeleteSofaMatch}
/>
...
<div
  ...
  style={{
    fontWeight: 'bold',
    fontSize: getFontSize(`${r.home} - ${r.away}`, 110)
  }}
>
