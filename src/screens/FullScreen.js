...
// Funkcija koja spaja duplikate u tabeli uz potvrdu korisnika
// eslint-disable-next-line no-unused-vars
const mergeTeamWithPrompt = (ligaTable, originalName, duplicateName) => {
  const originalTeam = ligaTable.find(t => t.team === originalName);
  const duplicateIndex = ligaTable.findIndex(t => t.team === duplicateName);

  if (!originalTeam || duplicateIndex === -1) return ligaTable;

  const duplicateTeam = ligaTable[duplicateIndex];

  const confirmMerge = window.confirm(`Da li želite da spojite "${duplicateTeam.team}" sa "${originalTeam.team}"?`);

  if (confirmMerge) {
    // Saberi statistike
    originalTeam.mp += duplicateTeam.mp;
    originalTeam.w += duplicateTeam.w;
    originalTeam.d += duplicateTeam.d;
    originalTeam.l += duplicateTeam.l;
    originalTeam.gf += duplicateTeam.gf;
    originalTeam.ga += duplicateTeam.ga;
    originalTeam.gd = originalTeam.gf - originalTeam.ga;
    originalTeam.pts += duplicateTeam.pts;

    // Izbriši duplikat
    ligaTable.splice(duplicateIndex, 1);
  }
...
const mergeTeams = (teamName) => {
  ...
  const confirmMerge = window.confirm(
    `Da li želiš da "${teamName}" postane "${standardName}"?`
  );
            ...
            {ligaObj.name} {confirmedLeagues[ligaObj.name] ? "✅" : ""}
                    ...
                    {openLeague === ligaObj.leagueId &&
                     ...
                     !ligaObj.name.toLowerCase().includes("liga konferencije") && (
                      <div>
               <button onClick={() => confirmLeagueTeams(ligaObj.name)}>
  ✅ Potvrdi listu timova
</button>
                        <table>
                          <thead>
                            <tr>
...
<td>{t.teamId}</td>
<td>
<button onClick={() => mergeTeams(t.team)}>Spoji</button>
</td>
