...
const patchPath = path.join(__dirname, "src/screens/Screen1.patch.js");

const newScreen1 = `import React, { useState, useContext, useRef, useCallback } from 'react';
...
export default function Screen1() {
  ...
  const addNewRow = () => {
    ...
    setScrollTop(0);
    setEditing({ row: 0, col: 'datum' }); // odmah edit na novom redu
  };
...
try {
  const backupPath = patchPath + ".backup." + Date.now();
  if (fs.existsSync(patchPath)) {
    fs.copyFileSync(patchPath, backupPath);
    console.log("📦 Backup originala napravljen:", backupPath);
  }
  fs.writeFileSync(patchPath, newScreen1, "utf8");
  console.log("✅ Screen1 PATCH je prepisan sa stabilnom verzijom.");
} catch(err) {
  console.error("❌ Greška pri pisanju PATCH fajla:", err.message);
}
